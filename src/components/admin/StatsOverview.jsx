...
const StatTile = ({ label, value, icon: Icon, color, bgColor, sub }) => (
  <div className="glass p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all group">
    ...
    <p className="text-2xl font-bold text-white group-hover:text-primary-400 transition-colors">{value}</p>
...
const PlanBar = ({ plan, count, total, color }) => {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-16 text-xs font-medium text-slate-300 shrink-0">{plan}</div>
      <div className="flex-1 bg-white/5 rounded-full h-2">
        <div className={`h-2 rounded-full ${color} transition-all duration-700`} style={{ width: `${pct}%` }} />
      </div>
      <div className="w-14 text-right text-xs text-slate-400 shrink-0">{count} ({Math.round(pct)}%)</div>
    </div>
  );
};
